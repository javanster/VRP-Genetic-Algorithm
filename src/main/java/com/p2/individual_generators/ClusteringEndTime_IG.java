...
/**
 * A class for creating an individual based on clustering and sorting after time window end time
 */
public class ClusteringEndTime_IG implements IndividualGenerator {
    ...
    public Individual generateIndividual(ParameterObject parameters, int clusterCount) {
        ...
        for (List<Integer> cluster : clusters) {
            cluster.sort((a, b) -> Integer.compare(problemInstance.getPatients().get(a).getEnd_time(), 
            problemInstance.getPatients().get(b).getEnd_time()));
        }
